<script lang="ts">
  ...
  const steps = [
    { title: 'Welcome to Horus', description: 'Analyze satellite images of your region with vegetation, soil, and climate tools.', icon: '🛰️' },
  ...
  function next() {
    ...
    else { localStorage.setItem('horus_onboarded', 'true'); handleClose(); }
  }
  function skip() { localStorage.setItem('horus_onboarded', 'true'); handleClose(); }
</script>
