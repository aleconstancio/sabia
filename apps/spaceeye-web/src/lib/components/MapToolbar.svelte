...
<div class="absolute left-4 top-1/2 -translate-y-1/2 z-[999] flex flex-col gap-2">
  ...
  <Button size="sm" variant="secondary" onclick={onMeasure} class="!w-10 !h-10 !p-0 !text-xs">
  ...
  <Button size="sm" variant="secondary" onclick={() => showBasemapMenu = !showBasemapMenu} class="!w-10 !h-10 !p-0 !text-xs">
...
{#if showBasemapMenu}
  <div class="absolute left-4 bottom-36 z-[999] rounded-lg border border-border bg-card p-2 shadow-lg">
    {#each ['satellite', 'osm', 'dark'] as b}
      ...
      >
        {b === 'satellite' ? 'Satélite' : b === 'osm' ? 'OpenStreetMap' : 'Escuro'}
      </button>
    {/each}
