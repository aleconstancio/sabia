...
export async function exportPdf(imageId: string, cloudCover: number | null) {
  ...
  try {
    await downloadBlobPost(`${API_URL}/export/pdf`, body, `sabia-${imageId.slice(0, 20)}.pdf`);
...
export async function downloadGeotiff(taskId: string) {
  try {
    await downloadBlob(`${API_URL}/download/${taskId}/geotiff`, `sabia-${taskId.slice(0, 8)}.tif`);
...
export async function exportEsgCsv(module: string, coordinates: number[][][]) {
  try {
    ...
    triggerDownload(blob, `sabia-${module}-export.csv`);
  } catch (e: unknown) {
