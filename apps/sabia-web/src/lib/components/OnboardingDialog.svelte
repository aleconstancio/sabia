<script lang="ts">
  ...
  const steps = [
    { title: 'Welcome to Sabiá', description: 'Analyze satellite images of your region with vegetation, soil, and climate tools.', icon: '🛰️' },
  ...
  function next() {
    ...
    else { localStorage.setItem('sabia_onboarded', 'true'); handleClose(); }
  }
  function skip() { localStorage.setItem('sabia_onboarded', 'true'); handleClose(); }
</script>
