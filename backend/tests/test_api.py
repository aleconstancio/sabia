...
@pytest.fixture
def client_with_search_db():
    """Mock DB for image search endpoints."""
    mock_session = AsyncMock()
    ...
    async def mock_execute(query, params=None):
        ...
        if "SELECT" in query_str and "COUNT" in query_str:
            ...
            result = MagicMock()
            # For get_image_by_id: return None (no row found) for nonexistent IDs
            if params and params.get("id", "").startswith("nonexistent"):
...
@pytest.mark.asyncio
async def test_health_endpoint(client_with_mock_db):
...
@pytest.mark.asyncio
async def test_images_search_invalid_polygon(client_with_search_db):
    """Single-point polygon: Pydantic accepts it, endpoint processes it via mocked DB."""
    resp = await client_with_search_db.post("/api/images/search", json={"coordinates": [[[0, 0]]]})
    assert resp.status_code == 200


@pytest.mark.asyncio
async def test_images_search_valid_polygon(client_with_search_db):
    poly = [[[-35.2, -5.8], [-35.1, -5.8], [-35.1, -5.7], [-35.2, -5.7], [-35.2, -5.8]]]
    resp = await client_with_search_db.post(
        "/api/images/search", json={"coordinates": poly, "limit": 5}
    )
    assert resp.status_code == 200
    data = resp.json()
...
@pytest.mark.asyncio
async def test_get_nonexistent_image(client_with_search_db):
    resp = await client_with_search_db.get("/api/images/nonexistent_id")
            ...
            json={
                ...
                "properties": {
                    "layers": [
                        ...
                        {"name": "oc", "depths": [{"values": {"mean": 15.0}}]},
                        {"name": "sand", "depths": [{"values": {"mean": 35.0}}]},
                        {"name": "silt", "depths": [{"values": {"mean": 40.0}}]},
                        {"name": "clay", "depths": [{"values": {"mean": 25.0}}]},
                    ]
                },
            },
        )
