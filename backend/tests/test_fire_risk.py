...
async def test_calculate_fire_risk_returns_valid_structure():
    ...
    with patch(
        "backend.services.fire_risk.fetch_weather_for_fire",
        new_callable=AsyncMock,
        return_value=mock_weather,
    ):
        result = await calculate_fire_risk(coords)

    assert "fire_risk_score" in result
    assert "risk_level" in result
    assert "factors" in result
    assert isinstance(result["fire_risk_score"], float)
    assert 0 <= result["fire_risk_score"] <= 100
...
async def test_calculate_fire_risk_high_when_hot_dry():
    ...
    assert result["fire_risk_score"] >= 60
